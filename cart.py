...
df = pd.read_csv("Bootcamp_miuul/machine_learning-220803-231749/machine_learning/datasets/diabetes.csv")
...
def plot_importance(model, features, num=len(X), save=False):            # ilk arguman model, 2. arguman feature=column 3. bicimlendirilmis ön tanımlı arguman 4. On tanımlı degeri False
                                                                         # featute_img ı kaydetmek istiyorsak save=True denmelidir.
    feature_imp = pd.DataFrame({'Value': model.feature_importances_, 'Feature': features.columns})  # feature lar bir df de tutuluyor
    plt.figure(figsize=(10, 10))
    sns.set(font_scale=1)
    sns.barplot(x="Value", y="Feature", data=feature_imp.sort_values(by="Value",            # kucukten buyuge dogru sıralayıp bar plot ile goster
                                                                     ascending=False)[0:num])  #num argumanının ön tanımlı degeri, özellik
                                                                                                # sayısı kadar gosterdir.
    plt.title('Features')
    plt.tight_layout()
    plt.show()
    if save:
        plt.savefig('importances.png')                                     # Degiskenlerin onem duzeylerini verir
...
def val_curve_params(model, X, y, param_name, param_range, scoring="roc_auc", cv=10):
    ...
    plt.title(f"Validation Curve for {type(model).__name__}")          # type(model).__name__ : modelin ismini dinamik bir sekilde almaya yarar
